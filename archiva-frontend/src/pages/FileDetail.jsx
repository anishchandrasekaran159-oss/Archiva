...
// ── Extension colour map — same as FileCard ──────────────────────────────────
const EXT_COLORS = {
...
function RecCard({ file, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className="card cursor-pointer flex gap-3 items-start hover:border-warm-accent transition-colors"
    >
      <FileTypeIcon filename={file.filename} size={36} />
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-medium text-ink-primary leading-snug mb-0.5 line-clamp-1">
          {file.filename}
        </p>
        {file.note && (
          <p className="text-[11px] text-ink-secondary line-clamp-2 leading-snug mb-1">
            {file.note}
          </p>
        )}
        <div className="flex items-center gap-2 flex-wrap">
          {file.subject && (
            <span className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-warm-tag text-warm-tag-text">
              {file.subject}
            </span>
          )}
...
export default function FileDetail() {
  ...
  const [data, setData]       = useState(null)  // { file, recommendations }
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-white border-[1.5px] border-warm-border rounded-xl p-6 mb-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              {file.subject && (
                <div>
                  <p className="text-[10px] font-medium text-ink-muted uppercase tracking-[0.6px] mb-1">
                    Subject
                  </p>
                  <span className="text-[12px] font-medium px-2.5 py-1 rounded-full
                                   bg-warm-tag text-warm-tag-text inline-block">
                    {file.subject}
                  </span>
                </div>
              )}
          ...
          {recs.length > 0 && (
            <>
              <div className="flex items-center gap-2 mb-3">
                <h2 className="text-[13px] font-medium text-ink-primary">
                  You might also need
                </h2>
